...
class ServiceManager:
    ...
    def __init__(self, socat_port: int = 8081, sleep_duration: int = 3) -> None:
        ...
        self.services: Dict[str, int] = {
            'chijoon_tec_be_1': 8080,
            'chijoon_tec_be_2': 8081
        }
    ...
    def _find_current_service(self) -> None:
        ...
        if not current_service:
            self.current_name, self.current_port = 'chijoon_tec_be_2', self.services['chijoon_tec_be_2']
        else:
    ...
    def _run_container(self, name: str, port: int) -> None:
        os.system(
            f"docker run -d --name={name} --restart unless-stopped -p {port}:8090 -e TZ=Asia/Seoul -v /dockerProjects/chijoon_tec/volumes/gen:/gen --pull always ghcr.io/kknaks/chijoontec_back")
